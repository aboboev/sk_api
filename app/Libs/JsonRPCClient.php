...
class JsonRPCClient
{
    ...
    public function __call($pMethod, $pParams)
    {
        ...
        if ($this->parameters_structure == 'object' && empty($pParams)) {
            $pParams = json_decode ('{}');
        }
...
//        }
        if (isset($responseDecoded['error']) && $responseDecoded['error'] != 0)
        {
    ...
    protected function & getResponse(&$pRequest)
    {
        ...
        {
            throw new \RuntimeException('Response Http Error - ' . $this->httpErrors[$httpCode]. ' '. $response);
        }
