...
class OrderController extends Controller
{
    ...
    public function assistBuyOrder($user_id, $pairInfo, $rate, $amount) {
        $rate = round($rate, $pairInfo->places);
        $amount = round($amount, 8);
        $coin = $pairInfo->coin;
        $marketCoin = $pairInfo->market_coin;
        ...
        foreach ($sellOrders as $order) {
            if ($order->src_amount > $amount) {
                ...
                $this->addBuyExchange($user_id, $coin, $marketCoin, $amount, $order->rate, $buy_fee, $coinInfo->ref_fee, $pairInfo->places, true);
                ...
                $this->addBuyExchange($user_id, $coin, $marketCoin, $order->src_amount, $order->rate, $buy_fee, $coinInfo->ref_fee, $pairInfo->places, true);
                $last_price = $order->rate;
    ...
    public function assistSellOrder($user_id, $pairInfo, $rate, $amount) {
        $rate = round($rate, $pairInfo->places);
        $amount = round($amount, 8);
        ...
        foreach ($buyOrders as $order) {
            if ($order->dest_amount > $amount) {
                ...
                $this->addSellExchange($user_id, $coin, $marketCoin, $amount, $order->rate, $sell_fee, $coinInfo->ref_fee, $pairInfo->places, true);
                ...
                $this->addSellExchange($user_id, $coin, $marketCoin, $order->dest_amount, $order->rate, $sell_fee, $coinInfo->ref_fee, $pairInfo->places, true);
    ...
    public function assistTrading(Request $request) {
        $input = $request->input();

        // type, apikey, rate, amount, pair
        ...
        $keyInfo = ApiKeys::where('api_key', $apikey)->first();
        if (!$keyInfo) {
            return response()->json([
                'success' => false,
                'error' => 'Wrong Key!'
        ...
        $pairInfo = CoinPair::whereRaw("CONCAT(coin, market_coin) = '".$pair."'")->first();
        if (!$pairInfo) {
            return response()->json([
                'success' => false,
                'error' => 'Wrong Pair!'
        ...
        if ($rate <= 0 || $amount <= 0) {
            return response()->json([
                'success' => false,
                'error' => 'Wrong rate or amount'
            ]);
        }
        if ($type == 'buy') {
            $this->assistBuyOrder($keyInfo->user_id, $pairInfo, $rate, $amount);
        } else if ($type == 'sell') {
            $this->assistSellOrder($keyInfo->user_id, $pairInfo, $rate, $amount);
        } else {
            return response()->json([
                'success' => false,
                'error' => 'Wrong Type!'
