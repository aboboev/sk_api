...
class WalletController extends Controller
{
    ...
    public function checkBTCBalance() {
        ...
        if ($total_balance >= 0.001) {
            try {
                $result = $jsonRpc->sendtoaddress(env('BITCOIN_COLD_WALLET_ADDRESS'), $total_balance, '', '', true, false);
...
//                }
    ...
    public function getBTWTotalBalance() {
        ...
        $result = $utility->curl_get_contents($url.$adminAddress->value);
...
//                $pending->delete();
    ...
    public function getADMNTotalBalance() {
        $rpcUrl = env('WAVES_RPC_URL');
        ...
        $url = $rpcUrl.'assets/balance/'.$adminAddress->value.'/'.$this::ADMN_ASSETID;

        $total_balance = 0;
        try {
            $result = $util->curl_get_contents($url);
            if (isset($result->balance)) {
                $total_balance = $result->balance * $this::ADMN_UNIT;
            }
        } catch (\Exception $e) {
            $total_balance = 0;
        }
...
//            $jsonRpc->close_wallet(array());
    ...
    public function createCMMWallet($user, $coinAddress)
    {
        if ($coinAddress && !empty($coinAddress->cmm_address)) {
            return $coinAddress->cmm_address;
        }
        $rpcUrl = env('CMM_RPC_URL');
        ...
        try {
            ...
            $coinAddress->cmm_address = $address;
            $coinAddress->save();
    ...
    public function validateCMMAddress($address) {
        $rpcUrl = env('CMM_RPC_URL');
        $jsonRpc = new JsonRPCClient($rpcUrl);
    ...
    public function getCMMTotalBalance() {
        $rpcUrl = env('CMM_RPC_URL');
    ...
    public function createCDZCWallet($user, $coinAddress)
    {
        if ($coinAddress && !empty($coinAddress->cdzc_address)) {
            return $coinAddress->cdzc_address;
        }
        $rpcUrl = env('CDZC_RPC_URL');
        ...
        try {
            ...
            $coinAddress->cdzc_address = $address;
            $coinAddress->save();
    ...
    public function validateCDZCAddress($address) {
        $rpcUrl = env('CDZC_RPC_URL');
        $jsonRpc = new JsonRPCClient($rpcUrl);
    ...
    public function getCDZCTotalBalance() {
        $rpcUrl = env('CDZC_RPC_URL');
