...
class IEOController extends Controller {
    ...
    public function getIEOAmountbyCoin($coin, Request $request) {
        ...
        $query =    "SELECT SUM(IF(src_coin = 'BTC', src_amount, 0)) btc_amount,  ".
                    ...
                    "FROM transactions ".
                    "WHERE type = {$icoType} AND dest_coin = '{$coin}' ".
                        "AND user_id IN (select id from users where role<>'assist') ";
    ...
    public function getIEOStatistics(Request $request) {
        ...
        foreach ($result as $record) {
            if ($coin == 'W12') {
                $data['ieo_amount'] = $record->ieo_amount - 671557 - 1007892;
            } else if ($coin == 'SRX') {
                $data['ieo_amount'] = $record->ieo_amount - 988645;
            } else if ($coin == 'LAAR') {
                $data['ieo_amount'] = $record->ieo_amount - 498140;
            } else {
    ...
    public function calcBonus($token, $usd_amount, $ieo_amount) {
        ...
        switch ($token) {
            ...
            case 'W12':
                if ($usd_amount >= 0) {
                    $bonus_amount = $ieo_amount * 6.52 / 100;
                }
                break;
            case 'XLMG':
            ...
            case 'WVR':
                if ($usd_amount >= 0) {
                    $bonus_amount = $ieo_amount * 25 / 100;
                }
                break;
            case 'SLO':
                if ($usd_amount >= 0) {
                ...
                break;
            case 'QPY':
                if ($usd_amount >= 100 && $usd_amount < 500) {
                    $bonus_amount = $ieo_amount * 25 / 100;
                } else if ($usd_amount >= 500 && $usd_amount < 1000) {
                    $bonus_amount = $ieo_amount * 45 / 100;
                } else if ($usd_amount >= 1000) {
                    $bonus_amount = $ieo_amount * 65 / 100;
                }
                break;
            case 'MOBA':
                if ($usd_amount >= 0) {
            ...
            case 'HELP':
                if ($usd_amount >= 100 && $usd_amount < 500) {
                    ...
                    $bonus_amount = $ieo_amount * 40 / 100;
                }
                break;
            case 'V4U':
                if ($usd_amount >= 0 && $usd_amount < 20) {
                    $bonus_amount = $ieo_amount * 5 / 100;
                } else if ($usd_amount >= 20 && $usd_amount < 50) {
                    $bonus_amount = $ieo_amount * 7.5 / 100;
                } else if ($usd_amount >= 50 && $usd_amount < 150) {
                    $bonus_amount = $ieo_amount * 15 / 100;
                } else if ($usd_amount >= 150) {
                    $bonus_amount = $ieo_amount * 30 / 100;
                }
                break;
            default: break;
    ...
    public function buyToken(Request $request) {
        ...
        $usd_amount = $ieo_amount * $usd_price;
