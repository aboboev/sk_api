...
{
    public function getDepositAmount($coin, $user) {
        $depositAmount = 0;
        ...
        switch ($coin) {
            case 'BTW':
                $depositAmount = $depositController->getBTWDeposit($coinAddress);
                break;
        }
    ...
    public function allBalance($user) {
        ...
        foreach ($result as $record) {
            $depositAmount = 0;
            if (env('APP_LOCAL') != 'local') {
                $depositAmount = $this->getDepositAmount($record->coin, $user);
            }

            $balance[$record->coin] = round($record->amount + $depositAmount, 8);
        }
    ...
    public function balance($user, $coin) {
        ...
        $depositAmount = $this->getDepositAmount($coin, $user);

        return round($tran_plus - $tran_minus - $order_plus + $depositAmount, 8);
    }
    ...
    public function requestWithdraw($coin, Request $request) {
        ...
        if (!$accountController->confirmAuthCode($user, $auth_code)) {
            return response()->json(['error' => 'Wrong Code', 'success' => false]);
        }
        ...
        if ($amount > 10000 && $coin == 'BEN') {
            return response()->json([
                'success' => false,
                'error' => 'Withdraw amount must be less than 10000 BEN'
            ]);
        }
    ...
    public function getDepositList($coin, Request $request) {
        ...
        switch ($coin) {
            case 'BTW':
                $result = $depositController->getBTWDepositList($coinAddress);
                break;
        }
        ...
        if (count($result) > 0) {
            $sort = new ArraySort();
            $result = $sort->array_sort($result, 'time', SORT_DESC);

            foreach ($result as $record) {
                array_push($data, array(
                    'type' => TransactionType::DEPOSIT,
                    'status' => $record['confirmations'] > 0 ? TransactionStatus::SUCCESS : TransactionStatus::PENDING,
                    'datetime' => Carbon::createFromTimestampUTC($record['time'])->toDateTimeString(),
                    'txid' => $record['txid'],
    ...
    public function transactionHistory($user, $coin) {
        ...
        if (count($result) > 0) {
            foreach ($result as $tran) {
                $record = array(
                    'type' => TransactionType::DEPOSIT,
                    'dest_amount' => $tran['amount'],
                    'txid' => $tran['txid'],
                    'datetime' => $tran['time'],
                    'dest_coin' => $coin,
                    'status' => $tran['confirmations'] == 0 ? TransactionStatus::PENDING : TransactionStatus::SUCCESS
                );
    ...
    public function getStatistics(Request $request) {
        ...
        foreach ($result as $record) {
            ...
            switch ($record->coin) {
                case 'BTW':
                    $depositAmount = $depositController->getBTWAllDeposit();
                    break;
            }

            $record->deposit_amount += $depositAmount;
